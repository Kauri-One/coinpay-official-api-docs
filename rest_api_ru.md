...
- [Заявки](#заявки)
  ...
  - [Заявка на депозит](#заявка-на-депозит)
    ...
    - [Уведомление по коллбеку](#уведомление-по-коллбеку)
...
### Уведомление по коллбеку

Если было пополнение по аддресу и для него указан был callback_url, то система по мере изменения статуса будет пытаться присылать нотификацию по этому url.
...
Условием, по которому система понимает что коллбек удачно обработан, это 200-ый ответ. Система пробует 3 раза отослать коллбек до получение 200-го ответа, с интервалом раз в 5 минут. Создаеться отдельная нотификация для каждого изменения статуса. 
